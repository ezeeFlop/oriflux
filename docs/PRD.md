...
- **Géo** : résolution IP → pays / région / ville / ASN via base MaxMind GeoLite2 embarquée (mise à jour automatique hebdo). L'IP est résolue à l'ingestion puis jetée — seules les dimensions géo sont stockées.
...
| Géolocalisation | MaxMind GeoLite2 (country+city+ASN), local | Gratuit, offline, RGPD-friendly (résolution à la volée, IP jetée). Clé MaxMind requise + EULA : pas de redistribution (cf. §14) |
...
| GeoLite2 : clé MaxMind requise, EULA interdit la redistribution | Faible | Refresh hebdo automatisé avec la clé SPT ; en phase 4, les clients self-hosted apportent leur propre clé gratuite (documenté dans l'installeur) |
